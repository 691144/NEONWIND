...
class GameEngine {
    ...
    private getMapAt(z: number): MapPoint {
        ...
        let idx = Math.floor(dist / MAP_RES);
        if (idx < 0) idx = 0;
        if (idx >= this.terrainMap.length) return this.terrainMap[this.terrainMap.length - 1] || {x:0, y:0};
        return this.terrainMap[idx];
    }
    ...
    private loop = () => {
        ...
        if (this.isControlsLocked) {
            ...
            this.plane.position.x = cx;
            this.plane.position.y = cy;
